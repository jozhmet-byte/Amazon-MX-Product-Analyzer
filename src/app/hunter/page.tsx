...
import { Search, Sparkles, TrendingUp, AlertCircle, Save, CheckCircle2, Loader2, Lock, ArrowRight } from "lucide-react";
...
export default function NicheHunterPage() {
  ...
  const handleSaveProduct = async (product: Product) => {
    if (!activeCampaign || savingAsin || savedAsins.has(product.asin)) return;
    ...
    try {
      const res = await fetch("/api/products/save", {
        ...
        body: JSON.stringify({
          ...
          project_name: activeCampaign // Guardado automático directo en la campaña activa
        })
      });
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full bg-[#0B0E14] text-center p-8">
        <div className="bg-[#1A1D24] p-8 rounded-2xl border border-zinc-800 shadow-2xl max-w-md w-full animate-in fade-in zoom-in-95 duration-200">
          ...
          <h2 className="text-xl font-bold text-white mb-2">Búsqueda Deshabilitada</h2>
          <p className="text-zinc-400 text-sm mb-6 leading-relaxed">
            Antes de buscar productos en Amazon MX, primero debes crear o activar una campaña desde tu gestor central.
          </p>
  ...
  return (
    <div className="flex flex-col h-full bg-[#0B0E14] overflow-y-auto">
      {/* Header */}
      <div className="bg-[#0F1111] px-8 py-6 border-b border-zinc-800 flex justify-between items-center">
        <div>
          ...
          <p className="text-zinc-400 text-sm">
            Buscando y guardando productos para la campaña activa: <strong className="text-white">"{activeCampaign}"</strong>
          </p>
        </div>
                      ...
                      <tr key={`${product.asin}-${index}`} className="border-b border-zinc-800 hover:bg-zinc-800/40 transition-colors">
                        ...
                        <td className="px-6 py-4 text-right">
                          <button
                            ...
                            title={savedAsins.has(product.asin) ? "Guardado en Tabla de Análisis" : "Guardar en Tabla de Análisis y Generar FODA"}
                          >
